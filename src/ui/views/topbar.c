...
static int TopBar_Update(Component *base) {
    ...
    if (IsMouseButtonReleased(MOUSE_BUTTON_LEFT)) {
        ...
        if (mouse.y < TOP_BAR_H) {
            ...
            if (mouse.x >= t->btnTagListX && mouse.x <= t->btnTagListX + t->btnTagListW) {
                if (t->OnTagList) t->OnTagList(t->callbackCtx);
            }
            if (mouse.x >= t->btnInfoX && mouse.x <= t->btnInfoX + t->btnInfoW) {
                if (t->OnInfo) t->OnInfo(t->callbackCtx);
            }
            if (mouse.x >= t->btnSettingsX && mouse.x <= t->btnSettingsX + t->btnSettingsW) {
                if (t->OnSettings) t->OnSettings(t->callbackCtx);
            }
        }
    }
    return 0;
...
static void TopBar_Draw(Component *base) {
    ...
    DrawRectangleLines(t->btnBrowseX, btnY, t->btnBrowseW, btnH, ColorShadow);
    {
        ...
        UIDrawText("BROWSE", faceSm, startX + iconW + spacing, btnY + S(2.5f), S(9), ColorWhite);
    }
    ...
    DrawRectangleLines(t->btnTagListX, btnY, t->btnTagListW, btnH, ColorShadow);
    {
        ...
        UIDrawText("TAG LIST", faceSm, startX + iconW + spacing, btnY + S(2.5f), S(9), ColorWhite);
    }
    ...
    DrawRectangleLines(t->btnInfoX, btnY, t->btnInfoW, btnH, ColorShadow);
    {
        ...
        UIDrawText("INFO", faceSm, startX + iconW + spacing, btnY + S(2.5f), S(9), ColorWhite);
    }
    ...
    DrawRectangleLines(t->btnSettingsX, btnY, t->btnSettingsW, btnH, ColorShadow);
    {
        ...
        UIDrawText("MENU", faceSm, startX + iconW + spacing, btnY + S(2.5f), S(9), ColorWhite);
    }
    ...
    if (t->BatteryLevel > 0) {
        ...
        if (t->BatteryLevel < 0.25f) fillColor = ColorRed;
        else if (t->BatteryLevel < 0.5f) fillColor = ColorOrange;

        for (int i = 0; i < segments; i++) {
            if (t->BatteryLevel >= (float)(i + 1) / segments - 0.05f) {
                DrawRectangle(batX + gap + i * (segW + segGap), batY + gap, segW, batH - (gap * 2), fillColor);
            }
        }
    }
