...
// By default, we use Touch_CheckClick because our new Touch Engine flawlessly handles BOTH Mouse and Touch inputs.
...
static inline Vector2 Input_GetPointerPos(void) {
    return Mouse_GetPos(); // Under the hood, this handles DRM touch-to-mouse tracking
}
...
static inline void Input_Consume(void) {
...
static inline Vector2 Input_GetStartPos(void) {
    return Touch_GetStartPos();
}
