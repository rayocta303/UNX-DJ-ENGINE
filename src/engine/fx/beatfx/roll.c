...
void Roll_Process(BeatFX_Roll *fx, float *outL, float *outR, float inL,
                  float inR, float beatMs, float levelDepth, float sampleRate,
                  bool isFxOn) {
  (void)beatMs;
  (void)sampleRate;

  // In Pioneer FX, Roll grabs the audio at the moment it's turned ON and loops
  // exactly that chunk.
  ...
  if (!fx->holding) {
    // Write continuously when OFF so we always have the past audio ready to
    // loop
