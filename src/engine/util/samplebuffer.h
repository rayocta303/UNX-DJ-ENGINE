...
class SampleBuffer final {
  public:
    ...
    std::span<CSAMPLE> span() noexcept {
        return unx::spanutil::spanFromPtrLen(m_data, m_size);
    }
    ...
    /// unx::spanutil::spanFromPtrLen from util/span.h instead.
...
template<>
inline void swap(::unx::SampleBuffer& lhs, ::unx::SampleBuffer& rhs) noexcept {
    lhs.swap(rhs);
}
