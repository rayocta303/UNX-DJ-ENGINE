...
class EngineBuffer : public EngineObject {
  ...
  public:
    ...
    unx::audio::ChannelCount getChannelCount() const {
        return m_channelCount;
    }
    unx::audio::FramePos getPlayPos() const {
        return m_playPos;
    }
    bool getScratching() const;
    bool isReverse() const;
    /// Returns current bpm value (not thread-safe)
    unx::Bpm getBpm() const;
    /// Returns the BPM of the loaded track around the current position (not thread-safe)
    unx::Bpm getLocalBpm() const;
    /// Sets a beatloop for the loaded track (not thread safe)
    void setBeatLoop(unx::audio::FramePos startPosition, bool enabled);
    /// Sets a loop for the loaded track (not thread safe)
    void setLoop(unx::audio::FramePos startPosition,
            unx::audio::FramePos endPositon,
            bool enabled);
    ...
    unx::audio::FramePos queuedSeekPosition() const;

    bool isTrackLoaded() const;
    TrackPointer getLoadedTrack() const;
    void ejectTrack();

    unx::audio::FramePos getExactPlayPos() const;
    double getVisualPlayPos() const;
    unx::audio::FramePos getTrackEndPosition() const;
    void setTrackEndPosition(unx::audio::FramePos position);
    ...
    void loadTrack(TrackPointer pTrack,
            unx::StemChannelSelection stemMask,
            bool play,
            EngineChannel* pChannelToCloneFrom);

    unx::StemChannelSelection getStemMask() const {
        return m_stemMask;
    }
#else
    void loadTrack(TrackPointer pTrack,
    ...
    void seekAbs(unx::audio::FramePos);
    void seekExact(unx::audio::FramePos);
  ...
  private slots:
    ...
    void slotTrackLoaded(
            TrackPointer pTrack,
            unx::audio::SampleRate trackSampleRate,
            unx::audio::ChannelCount trackChannelCount,
            unx::audio::FramePos trackNumFrame);
  ...
  private:
    ...
    double fractionalPlayposFromAbsolute(unx::audio::FramePos position);

    void doSeekFractional(double fractionalPos, enum SeekRequest seekType);
    void doSeekPlayPos(unx::audio::FramePos position, enum SeekRequest seekType);
    ...
    void setNewPlaypos(unx::audio::FramePos playpos);
    ...
    unx::audio::FramePos m_trackEndPositionOld;

    // Copy of file sample rate
    unx::audio::SampleRate m_trackSampleRateOld;
    ...
    static constexpr QueuedSeek kNoQueuedSeek = {unx::audio::kInvalidFramePos, SEEK_NONE};
    /// indicates a clone seek on a bosition from another deck
    static constexpr QueuedSeek kCloneSeek = {unx::audio::kInvalidFramePos, SEEK_CLONE};
    ...
    unx::audio::SampleRate m_sampleRate;

    // The current channel count of the loaded track
    unx::audio::ChannelCount m_channelCount;
...
#ifdef __STEM__
    unx::StemChannelSelection m_stemMask;
#endif
};
