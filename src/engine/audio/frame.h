...
class FramePos final {
  public:
    ...
    static constexpr FramePos fromEngineSamplePos(double engineSamplePos) {
        return FramePos(engineSamplePos / unx::kEngineChannelOutputCount);
    }
    ...
    static constexpr FramePos fromSamplePos(double samplePos,
            const unx::audio::SignalInfo& signalInfo) {
    ...
    double toEngineSamplePos() const {
        ...
        double engineSamplePos = value() * unx::kEngineChannelOutputCount;
    ...
    double toSamplePos(unx::audio::ChannelCount channelCount) const {
        DEBUG_ASSERT(isValid());
    ...
    static constexpr FramePos fromSamplePosMaybeInvalid(
            double samplePos, unx::audio::ChannelCount channelCount) {
    ...
    double toSamplePosMaybeInvalid(unx::audio::ChannelCount channelCount) const {
        if (!isValid()) {
            return kLegacyInvalidEnginePosition;
        }
...
Q_DECLARE_TYPEINFO(unx::audio::FramePos, Q_MOVABLE_TYPE);
Q_DECLARE_METATYPE(unx::audio::FramePos);
