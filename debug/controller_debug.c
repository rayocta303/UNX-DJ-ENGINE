...
static void ProcessIncomingMIDI(uint8_t b1, uint8_t b2, uint8_t b3) {
  ...
  printf("[%06u] | Status: 0x%02X (%s) | Ch: %02d | Ctrl/Note: 0x%02X (%3d) | "
         "Val: 0x%02X (%3d | %0.2f)",
         g_msgCounter, b1, typeName, ch, b2, b2, b3, b3, b3 / 127.0f);

  if (match) {
    printf(" -> Mapped: %s %s [%s]", match->group, match->key,
           match->options[0] ? match->options : "Normal");
  } else {
    printf(" -> [Unmapped]");
  }
  printf("\n");
...
static void ListDevices(void) {
  ...
  for (UINT i = 0; i < numInDevs; i++) {
    ...
    if (midiInGetDevCaps(i, &caps, sizeof(MIDIINCAPS)) == MMSYSERR_NOERROR) {
      printf("  [%u] %s %s\n", i, caps.szPname,
             (int)i == g_currentInDevId ? "(CONNECTED)" : "");
    }
  }
  ...
  for (UINT i = 0; i < numOutDevs; i++) {
    ...
    if (midiOutGetDevCaps(i, &caps, sizeof(MIDIOUTCAPS)) == MMSYSERR_NOERROR) {
      printf("  [%u] %s %s\n", i, caps.szPname,
             (int)i == g_currentOutDevId ? "(CONNECTED)" : "");
    }
  }
#else
...
static bool ConnectDevice(int devId) {
  ...
  MIDIINCAPS capsIn;
  if (midiInGetDevCaps((UINT)devId, &capsIn, sizeof(MIDIINCAPS)) !=
      MMSYSERR_NOERROR) {
    printf("[ERROR] Invalid MIDI Input Device ID: %d\n", devId);
    return false;
  }

  if (midiInOpen(&g_hMidiIn, (UINT)devId, (DWORD_PTR)MidiInProc, 0,
  ...
  for (UINT o = 0; o < numOut; o++) {
    MIDIOUTCAPS capsOut;
    if (midiOutGetDevCaps(o, &capsOut, sizeof(MIDIOUTCAPS)) ==
        MMSYSERR_NOERROR) {
      if (strstr(capsOut.szPname, capsIn.szPname) ||
          strstr(capsIn.szPname, capsOut.szPname)) {
        if (midiOutOpen(&g_hMidiOut, o, 0, 0, CALLBACK_NULL) ==
            MMSYSERR_NOERROR) {
...
static void TestVUMeters(void) {
  printf("\n[TEST] Running VU Meter LED Ramp Test (Channels 1-4 & Master "
         "L/R)...\n");
  // Handshake
...
static void TestJogRings(void) {
  printf("\n[TEST] Running Jog Wheel Ring Animation Test (Decks 1-4, Status "
         "0xBB)...\n");
...
static void PrintHelp(void) {
  ...
  printf("  list                          : List available MIDI Input & Output "
         "devices\n");
  printf(
      "  connect <id>                  : Connect to MIDI Controller by ID\n");
  printf("  load <path_to_xml>            : Load XML mapping preset (e.g. load "
         "../controllers/Pioneer-DDJ-FLX6.midi.xml)\n");
  printf("  monitor                       : Toggle real-time incoming MIDI "
         "message logging\n");
  printf("  send <status_hex> <b2> <b3>   : Send short MIDI msg to output "
         "(e.g. send 90 0B 7F)\n");
  printf("  test_vu                       : Run automated VU Meter LED ramp "
         "test (Ch 1-4 & Master)\n");
  printf("  test_jog                      : Run automated Jog Wheel Ring LED "
         "rotation test (Decks 1-4)\n");
  printf("  test_pads                     : Run automated HotCue Pad LED chase "
         "test (Decks 1-4)\n");
  printf("  test_all                      : Run full hardware LED diagnostic "
         "suite\n");
  printf("  sysex_flx6_keepalive          : Send Pioneer DDJ-FLX6 keep-alive "
         "SysEx\n");
  ...
  while (1) {
    ...
    if (strcmp(cmd, "exit") == 0 || strcmp(cmd, "quit") == 0) {
      ...
      if (strlen(arg1) == 0) {
        ...
        if (LoadXMLMapping(arg1, &g_activeMapping)) {
          ...
          printf("[SUCCESS] Loaded XML Mapping '%s' (%d controls) by %s\n",
                 g_activeMapping.name, g_activeMapping.count,
                 g_activeMapping.author);
        } else {
      ...
      printf("[MONITOR] Real-time MIDI Logging is now %s\n",
             g_monitoring ? "ACTIVE (Press 'monitor' to toggle off)"
                          : "PAUSED");
    } else if (strcmp(cmd, "send") == 0) {
      if (strlen(arg1) == 0 || strlen(arg2) == 0 || strlen(arg3) == 0) {
        printf("Usage: send <status_hex> <control_hex> <value_hex>  (e.g. send "
               "90 0B 7F)\n");
      } else {
      ...
      printf("[SYSEX] DDJ-FLX6 Keep-Alive SysEx string: F0 00 40 05 00 00 04 "
             "05 00 50 02 F7\n");
